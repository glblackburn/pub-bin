...
try:
    # Rich library imports available but not currently used
...
class TcpdumpParser:  # pylint: disable=too-few-public-methods
    ...
    def _detect_protocol(self, line: str) -> str:
        ...
        if self.PROTOCOL_PATTERNS['ICMP'].search(line):
            return 'ICMP'
        if self.PROTOCOL_PATTERNS['UDP'].search(line):
            return 'UDP'
        if self.PROTOCOL_PATTERNS['QUIC'].search(line):
            return 'QUIC'
        if self.PROTOCOL_PATTERNS['TCP'].search(line):
            return 'TCP'
        return 'UNKNOWN'
...
class ConnectionAnalyzer:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        ...
        # Connection key: (src_ip, dst_ip, protocol, src_port, dst_port)
        self.connection_counts: Dict[
            Tuple[str, str, str, Optional[int], Optional[int]], int
        ] = defaultdict(int)
    ...
    def get_connection_counts(
        self
    ) -> Dict[Tuple[str, str, str, Optional[int], Optional[int]], int]:
        """
...
def generate_connections_report(analyzer: ConnectionAnalyzer, n: int = 20) -> str:
    ...
    if not connections:
        ...
        for (src_ip, dst_ip, protocol, src_port, dst_port), count in sorted_connections[:n]:
            ...
            lines.append(
                f"  {src_str} -> {dst_str} ({protocol}) - {count:,} packets"
            )

    return "\n".join(lines)


def generate_ports_report(
    analyzer: ConnectionAnalyzer, n: int = 20, direction: str = 'source'
) -> str:
    """
...
def output_csv(analyzer: ConnectionAnalyzer, output_format: str) -> str:
    ...
    if output_format == 'ips':
        ...
        writer.writerow(
            ['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol', 'count']
        )
        conn_counts = analyzer.get_connection_counts()
        for (src_ip, dst_ip, protocol, src_port, dst_port), count in conn_counts.items():
...
def main():  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Analyze tcpdump output files to extract IP addresses '
...
Examples:
  ...
  %(prog)s -c -o connections
    ...
    parser.add_argument(
        '-f', '--file', type=str, action='append', dest='files',
        help='Input tcpdump file (can be specified multiple times)'
    )
    parser.add_argument(
        '-o', '--output-format', type=str, default='all',
        choices=['summary', 'ips', 'connections', 'ports', 'all'],
        help='Output format (default: all)'
    )
    parser.add_argument(
        '-p', '--protocol', type=str, default='all',
    ...
    parser.add_argument(
        '-l', '--exclude-local', action='store_true',
