...
- **Automated (in-repo):** [`osx/tests/test_def009_rich_table_layout_pty.py`](../../../osx/tests/test_def009_rich_table_layout_pty.py) + [`osx/tests/def009_layout_heuristics.py`](../../../osx/tests/def009_layout_heuristics.py). Heuristics cover **structural** fusion (``╭`` on the same stripped line as inner heavy **━** U+2501), **literal** doubled column rules, and **stderr JSON** merged into a table row (when not using **`ignore_merged_debug_tui_rows`** — pexpect merges stderr into the stdout transcript when **`MACOS_MOUSE_CLICK_DEBUG_TUI=1`**, which is not Rich corruption). The **darwin** + **`table_nav`** PTY test asserts ``layout_corruption_reason(..., ignore_merged_debug_tui_rows=True) is None`` at **40×120** with debug TUI enabled. Synthetic fixtures keep the detector honest if the layout regresses.
