...
Detail files for **DEF-001**–**DEF-011**. The **Defect summary** table (status, fix SHAs, manual verification) remains canonical in **[`../plans/plan-002-macos-mouse-click-terminal-ux.md`](../plans/plan-002-macos-mouse-click-terminal-ux.md)**; update the table and the matching **`def-###`** file together when closing a defect.
...
| DEF-011 | Mouse-move abort annulus: default arm radius > threshold → false stop on buy ladder | 2026-04-26 | — | **Open** | [def-011-mouse-move-abort-arm-threshold-annulus.md](def-011-mouse-move-abort-arm-threshold-annulus.md) |
