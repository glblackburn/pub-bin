...
todos:
  - id: "plan-doc"
    content: "Add this plan file under docs/osx/plans/agent/ (no code changes in same commit if split)"
    status: completed
  - id: "runner-up-arrow"
...
- **Factor shared PTY helpers** into `osx/tests/` only after runners stabilize (aligns with [`plan-agent-osx-dry-refactor.plan.md`](plan-agent-osx-dry-refactor.plan.md)).
