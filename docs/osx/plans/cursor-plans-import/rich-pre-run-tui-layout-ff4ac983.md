...
1. **Match reporter conditions:** same Terminal profile, font, line spacing; `./osx/macos_mouse_click.py --learn --interactive` (or minimal flags from hand-off); toggle `MACOS_MOUSE_CLICK_DEBUG_TUI`.
...
4. **Docs:** If DEF-010 is now a first-class defect narrative, add [`docs/osx/defects/def-010-*.md`](docs/osx/defects/) or trim stale references in module strings (hand-off item 4). Update DEF-009 / plan-002 defect summary only if behavior or mitigation changed materially.
...
- After implementation, store the **canonical** agent session plan under [`docs/osx/plans/agent/`](docs/osx/plans/agent/) with a `plan-agent-` prefix per [`docs/osx/plans/agent/README.md`](docs/osx/plans/agent/README.md) (this CreatePlan output can be copied there).
