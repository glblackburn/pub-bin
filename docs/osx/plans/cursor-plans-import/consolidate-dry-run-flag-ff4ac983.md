...
isProject: false
---
# Consolidate dry-run into `--dry-run`
...
- Update [`docs/osx/plans/`](docs/osx/plans/) references that mention `--dry-run-after-start` or split “after editor / after start” hooks so they describe **`--dry-run`** and the phased semantics above.
- Per repo rules, store any session-specific agent plan under [`docs/osx/plans/agent/`](docs/osx/plans/agent/) with prefix `plan-agent-` and link from that folder’s README if this becomes the canonical write-up.
