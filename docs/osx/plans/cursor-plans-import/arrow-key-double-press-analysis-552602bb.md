...
isProject: false
---
# Arrow Up/Down “double press” — log + code analysis (no code changes)

## What you were asked to run (reproduction)
...
We did **not** execute this in plan mode; the conclusions below come from **reading** [`osx/macos_mouse_click.py`](osx/macos_mouse_click.py) and [`osx/tests/_scratch_phase1_rich_table_pty.md`](osx/tests/_scratch_phase1_rich_table_pty.md).
...
## What to look for in `debug.json` (your run)
...
5. **Tests:** Extend PTY coverage (plan Phase 3 direction) so arrow bytes are injected deterministically and **`debug.json`** is parsed for `draw`/`after_key` correlation (partially prepared in local edits to [`osx/tests/test_rich_table_nav_down_pty.py`](osx/tests/test_rich_table_nav_down_pty.py), not required for this analysis).
...
**Next step:** Run your command once, reproduce one Down, and classify **Case A vs B** from `last_key` in `debug.json`. That determines whether the first fix is **logging/order** (misread) or **stdin decoding / drain / CSI** (real double-press).
