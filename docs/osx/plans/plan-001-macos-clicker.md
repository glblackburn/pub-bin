...
**TTY UX overlay (colors + pre-run editor):** see **[`plan-002-macos-mouse-click-terminal-ux.md`](plan-002-macos-mouse-click-terminal-ux.md)**. Plan 01 remains the source of truth for click semantics; plan 02 covers Rich-based presentation on an interactive terminal. **What shipped and what to verify by hand** is summarized in plan 02 under **[Implementation status](plan-002-macos-mouse-click-terminal-ux.md#implementation-status)**.
...
- Optional: commit **`docs/osx/plans/plan-001-macos-clicker.md`** with the script.
