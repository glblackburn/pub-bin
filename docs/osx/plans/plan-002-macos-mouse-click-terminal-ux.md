...
| DEF-011 | 2026-04-26 | **Fixed** (script) | **DEF-010 follow-on:** default **arm radius** (e.g. 60 px @ threshold 20) **>** leave threshold — same tick **arm + abort** before first click on sequential ladder rows ~63 px apart; **fix:** defer leave check until **`n_done > 0`** | Cookie Clicker / `macos_mouse_click_loop.sh`; `run_synthetic_loop` | `8e2843c45cc5074c4dc7dc4159f6be01f906f361` | **Passed** (automated: `test_mouse_move_abort.py`) |
