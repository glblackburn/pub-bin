...
**Open:** **`cookie_clicker_golden_sweeper.py`** is invoked only after the **inter-phase** sleep branch (**`i < k`**), so **`-k 1`** never runs the sweeper; **`CYCLE_SLEEP_SECONDS`** is also used between **outer** cycles. Normative detail: **[DEF-014](../defects/def-014-golden-sweeper-loop-sleep-placement-k1.md)**; **[plan-015](../plans/plan-015-cookie-clicker-golden-cookie-sweeper.md)** (looper §7).
