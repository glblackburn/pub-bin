...
Options
  ...
  -P <file>     : Coordinate profile JSON (from cookie_clicker_detect_coords.py or a
                 coords-only file). Preview is skipped when source_image is missing,
                 empty, builtin, or not a readable file path (see osx/README.md).
...
raw_si = data.get("source_image", "")
if raw_si is None:
    si_st = ""
else:
    si_st = str(raw_si).strip()
if not si_st or si_st == "builtin":
    coords_only = True
else:
    coords_only = not os.path.isfile(os.path.abspath(si_st))
print(f"COORDS_ONLY_PROFILE={'true' if coords_only else 'false'}")
...
function render_preview_artifacts {
    ...
    if [ "${COORDS_ONLY_PROFILE}" == true ]; then
        return
    fi
    preview_paths
    preview_args=()
...
# Explicit -P that resolves to the same file as the built-in default: treat as no -P
...
if [ ! -z "${profile_json}" ] && [ "${COORDS_ONLY_PROFILE}" != true ] && [ ! -f "${preview_script}" ]; then
    usage "Preview helper not found: ${preview_script}"
    exit 1
fi

if [ "${PREVIEW_ONLY}" == true ] && [ ! -z "${profile_json}" ] && [ "${COORDS_ONLY_PROFILE}" == true ]; then
    echo "Error: -N (preview only) requires a profile with a drawable source_image (not missing, empty, builtin, or absent on disk). Use -D with a screenshot first, or use a profile produced by cookie_clicker_detect_coords.py." >&2
    exit 2
fi

if [ "${REQUIRE_PREVIEW}" == true ] && [ "${COORDS_ONLY_PROFILE}" == true ]; then
    usage "-R requires a profile with a drawable source_image; coords-only profiles (missing/empty/builtin source_image, or path not found on disk) cannot produce a preview manifest."
    exit 1
fi
