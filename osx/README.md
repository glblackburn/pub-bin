...
`make -C osx` targets (`setup`, `detect-cookie-clicker`, `preview-cookie-clicker`) already use the venv Python. **`make -C osx setup`** depends on the **`tesseract`** target: on **macOS** its recipe is **`command -v tesseract || brew install tesseract`**, so **Homebrew runs only when `tesseract` is not already on `PATH`**; on other OSes **`tesseract`** is a no-op (install your distro’s `tesseract` if you use OCR).

Helper scripts:

- **`osx/cookie_clicker_detect_coords.py`**: creates profile JSON with cookie + ladder coordinates and confidence metadata. **Store row Y** defaults to **Tesseract OCR** on the detected store crop (English building names → row centers; missing rows interpolated). Requires the **`tesseract`** binary on **`PATH`** (e.g. `brew install tesseract`) plus **`pytesseract`** from **`osx/requirements.txt`**. Use **`--no-ocr`** to force the legacy edge-density ladder only (CI / regression).
...
- **`osx/cookie_clicker_golden_sweeper.py`**: plan-015 — poll for golden-ish blobs (HSV **v2** geometry filters + top-**K** cap), emit **`x y`** / JSONL (see **`docs/osx/plans/plan-015-cookie-clicker-golden-cookie-sweeper.md`**). Executable like the other `osx/*.py` helpers (shebang + **`osx/.venv`** re-exec). **`--capture display`** uses **`screencapture`**; by default each poll’s **raw** PNG is written under **`docs/osx/screenshots/golden-sweeper-captures/`** (that path is **gitignored** — local captures only). When hits are found, a second **`…-annotated.png`** holds boxes, centroids, and **confidence** labels (raw file unchanged). A **JSONL** sidecar **`{raw-basename}.json`** (one object per line: `x`, `y`, `confidence`, etc.) is written beside the **raw** PNG (or beside **`--input-image`**) when there is **at least one** hit; **`--emit-empty-json`** also writes an **empty** sidecar on no-hit polls. Use **`--no-capture-save`** for temp-only captures. Example: `./osx/cookie_clicker_golden_sweeper.py --capture display --dry-run --max-wall-seconds 5 --output json`. Optional **`--min-confidence`** and **`--det-*`** overrides tune `detect_magic_cookie_hits` without editing code (see **plan-018**). Optional corpus review: **`docs/osx/golden-sweeper-corpus-INDEX.md`** (run **`./osx/.venv/bin/python3 tools/build_golden_sweeper_corpus_index.py`** after collecting PNGs).
- **`tools/eval_magic_cookie_labels.py`**: plan-017 / **plan-018** — offline metrics vs **`magic-cookie-labels.jsonl`** (IoU / recall@**K** on positives, FP rate on negatives, optional **`--write-debug-dir`** overlays with ground-truth magenta box). Example: `./osx/.venv/bin/python3 tools/eval_magic_cookie_labels.py --labels docs/osx/screenshots/golden-sweeper-captures/magic-cookie-labels.jsonl`
- **`osx/magic_cookie_label_tool.py`**: plan-016 — PySide6 UI to classify sweeper PNGs (**magic cookie** yes / no / skip), draw a bounding box when yes, and append **`docs/osx/screenshots/golden-sweeper-captures/magic-cookie-labels.jsonl`** by default (same directory as the default **`--image-dir`** corpus). Requires **`PySide6`** from **`osx/requirements.txt`**. **Find image:** **Ctrl+F** (substring or full path), **F3** (next match), **`--jump-query SUBSTRING`** to open on the first matching PNG (plan-019). Example: `./osx/magic_cookie_label_tool.py --jump-query 20260504-223259-223436-f00001`. See **`docs/osx/plans/plan-016-magic-cookie-screenshot-label-tool.md`** and **`docs/osx/plans/plan-019-magic-cookie-label-tool-find-image.md`**.

**Roadmap:** active plan: **[plan-020 — uber true-up](../docs/osx/plans/plan-020-uber-true-up.md)** (the only open plan; all other plans are frozen and read-only). For shipped behavior, see the frozen reference plans: [plan-001](../docs/osx/plans/plan-001-macos-clicker.md) (clicker semantics), [plan-002](../docs/osx/plans/plan-002-macos-mouse-click-terminal-ux.md) (Rich pre-run UX), [plan-014](../docs/osx/plans/plan-014-macos-mouse-click-loop-cookie-before-ladder.md) (`-k` phased bursts), [plan-016](../docs/osx/plans/plan-016-magic-cookie-screenshot-label-tool.md) (PySide labeler), [plan-019](../docs/osx/plans/plan-019-magic-cookie-label-tool-find-image.md) (label tool find image). **`cookie_clicker_golden_sweeper.py`** is still a **v0 heuristic** (HSV blobs); **`macos_mouse_click_loop.sh`** invokes it once per cycle after cookie bursts (dry-run, short wall clock; see **DEF-014**). Further looper hooks (plan-015 §7.3–§7.5) and detector tuning are tracked under plan-020's checklist (`GS-15.7.x`, `DE-17.8.x`, `DE-18`).
