#!/usr/bin/env bash
# Test runner for load-ssh-key.sh tests
...
Run BATS tests for load-ssh-key.sh
...
Examples:
  ...
  $0 -f "k option"    # Run only tests matching "k option"
...
main() {
    echo "=========================================="
    echo "load-ssh-key.sh Test Suite"
    echo "=========================================="
    echo ""
    ...
    # BATS outputs "1..N" at the start to indicate total number of tests
    local total_line=$(echo "$bats_output" | grep -E "^[0-9]+\.\.[0-9]+" | head -1)
    local total_tests=0
    if [ -n "$total_line" ]; then
        # Extract the number after ".." (e.g., "1..27" -> "27")
    ...
    # Check for test failures in output (BATS outputs "not ok" for failed tests)
